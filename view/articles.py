...
def add_article():
    if not 'name' in session and 'password' in session :
        raise PermissionError('you need to login first')

    if not check_user_in_data(session.get('name') , session.get('password')):
        raise PermissionError('you need to create account first')
...
def update_article(article_id):
    ...
    user_id = get_user_Data(session.get('name'), session.get('password'))
